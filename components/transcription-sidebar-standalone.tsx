...
// Removed Stream SDK hooks for standalone testing
...
export const TranscriptionSidebarStandalone = ({ onClose }: TranscriptionSidebarProps) => {
  ...
  // Handle Screen Share Audio Mixing (Deepgram Only) - Mocked to do nothing
  useEffect(() => {
    if (engine === 'pro') {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (webSpeechService.isActive()) webSpeechService.stop();
    };
  }, [engine]);
  ...
  return (
    <div className="ml-2 flex h-[calc(100vh_-_86px)] w-80 flex-col rounded-lg border border-dark-1 bg-dark-1 p-4">
      ...
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-bold text-white">Live Transcription (Debug)</h2>
        <button
          onClick={onClose}
